...
class FLoader
{
    ...
    {
        echo '[TEST TRACE]', "FLoader::register()\n";
    ...
    {
        echo '[TEST TRACE]', "FLoader::loader($className)\n";
        $ns = substr($className, 0, strrpos($className, '\\'));
        echo '[TEST TRACE]', "FLoader::loader() ns: $ns \n";
        if(!empty($ns)){
            $className = strrchr($className, '\\');
            $className = substr($className, 1);
            echo '[TEST TRACE]', "FLoader::loader() className: $className \n";
    ...
    {
        echo '[TEST TRACE]', "FLoader::_loadClassByFile($filePath, $className)\n";
        ...
        if(!$isFound && !empty($this->_basePath) && substr($filePath, 0, 1) != DIRECTORY_SEPARATOR){
            ...
            echo '[TEST TRACE]', "FLoader::_loadClassByFile() filePath: $filePath \n";
            if(file_exists($filePath))
                $isFound = true;
        }
        ...
        echo '[TEST TRACE]', "FLoader::_loadClassByFile() rule: " . $filePath . DIRECTORY_SEPARATOR . eval($this->_rule) . "\n";
